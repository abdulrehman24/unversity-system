            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="">
                        <div class="row">
                            <div class="col-md-6">
                                <p><mark class="text-primary">{{ __('field_phone') }}:</mark> {{ $row->phone }}</p>
                                <hr />
                                <p><mark class="text-primary">{{ __('field_email') }}:</mark> {{ $row->email }}</p>
                                <hr />
                                <p><mark class="text-primary">{{ __('field_address') }}:</mark> {{ $row->address }}</p>
                                <hr />

                                <p><mark class="text-primary">{{ __('field_reference') }}:</mark>
                                    {{ $row->reference->title ?? '' }}</p>
                                <hr />
                                <p><mark class="text-primary">{{ __('field_source') }}:</mark>
                                    {{ $row->source->title ?? '' }}</p>
                                <hr />

                                @php
                                    $purposes = json_decode($row->purpose, true); // decode to array
                                @endphp

                                <p>
                                    @if (is_array($purposes))
                                        @foreach ($purposes as $index => $purpose)
                                            <mark class="text-primary">Follow Up {{ $index + 1 }}:</mark> {!! $purpose !!}<br>
                                        @endforeach
                                    @else
                                ...
                                <hr />
                                <p><mark class="text-primary">{{ __('field_note') }}:</mark> {!! $row->note !!}
                                </p>
                                <hr />
                            </div>
                            <div class="col-md-6">
                                <p><mark class="text-primary">{{ __('field_program') }}:</mark>
                                    {{ $row->program->title ?? '' }}</p>
                                <hr />
                                <p><mark class="text-primary">{{ __('field_assigned') }}:</mark>
                                    #{{ $row->assign->staff_id ?? '' }} - {{ $row->assign->first_name ?? '' }}
                                    {{ $row->assign->last_name ?? '' }}</p>
                                <hr />
                                ...
                                <p><mark class="text-primary">{{ __('field_recorded_by') }}:</mark>
                                    #{{ $row->recordedBy->staff_id ?? '' }}</p>
                                <hr />
                            </div>
                        </div>
                    </div>
